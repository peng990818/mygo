...
func walkSelectCases(cases []*ir.CommClause) []ir.Node {
    ...
    if ncas == 0 {
        // 0个case的时候，编译器会生成一个block调用
        // 没有case的select语句会被永远阻塞
    ...
    if ncas == 1 {
        ...
        if cas.Comm != nil { // not default:
            ...
            case ir.OSELRECV2:
                // 如果是接收操作且有两个返回值 (OSELRECV2)，编译器需要检查并处理可能的特殊情况（如接收值或判断通道关闭的布尔值被忽略的情况），
                // 并将操作类型从 OSELRECV2 转换为 OAS2RECV
        ...
        // 处理完 Comm 操作后，将 case 的 Body（case 内的语句块）添加到结果列表中，最后添加一个 break 语句以确保 select 语句结束后跳出。
        l = append(l, cas.Body...)
