...
type Builder struct {
    ...
    actionCache        map[cacheKey]*Action      // a cache of already-constructed actions  缓存一些已经构建过的动作
    mkdirCache         map[string]bool           // a cache of created directories 创建的目录的缓存
    flagCache          map[[2]string]bool        // a cache of supported compiler flags 支持的编译器标志的缓存
